//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

@interface LCFileStorage : NSObject
{
}

- (id)readStringWithJSFileName:(id)arg1;
- (_Bool)writeWithJSFileName:(id)arg1 object:(id)arg2;
- (id)getJSFilePath:(id)arg1;
- (id)cacheDirectory;
- (id)documentDirectory;
- (id)getFolderPath:(id)arg1;
- (id)getFilePath:(id)arg1;
- (_Bool)createFolderAtPath:(id)arg1;
- (void)deleteWithFileName:(id)arg1;
- (id)readArrayWithFilePath:(id)arg1;
- (id)readArrayWithFileName:(id)arg1;
- (id)readDataWithFilePath:(id)arg1;
- (id)readDataWithFileName:(id)arg1;
- (id)readStringWithFilePath:(id)arg1;
- (id)readStringWithFileName:(id)arg1;
- (id)dictionaryWithContentsOfFile:(id)arg1;
- (id)dictionaryWithContentsOfFileName:(id)arg1;
- (_Bool)writeWithFilePath:(id)arg1 object:(id)arg2;
- (_Bool)writeWithFileName:(id)arg1 object:(id)arg2 folderPath:(id)arg3;
- (_Bool)writeWithFileName:(id)arg1 object:(id)arg2;
- (void)modifyDictionaryWithContentsOfFileName:(id)arg1 handler:(CDUnknownBlockType)arg2;
- (void)delete_article:(id)arg1 src:(int)arg2 folderPath:(id)arg3;
- (void)insert_article_object:(id)arg1 channelId:(id)arg2 src:(int)arg3 folderPath:(id)arg4;
- (id)article_object:(id)arg1 src:(int)arg2 folderPath:(id)arg3;
- (long long)article_object_update:(id)arg1 src:(int)arg2 folderPath:(id)arg3;

@end

